# macOS 使用华为手机调试 Flutter：ADB 设备为空排查

> 记录在 macOS 上连接华为 Android 真机运行 Flutter 项目的完整过程，包含 ADB 无线配对与连接、USB 调试、MTP 模式、adb devices 为空及首次 Gradle 构建排查。
...
<alert title="建议首次连接使用 USB" type="info">
...
<alert title="配对端口和连接端口不同" type="warning">
...
<alert title="充电正常不代表数据正常" type="warning">
...
<alert title="macOS 通常不需要安装华为驱动" type="info">
